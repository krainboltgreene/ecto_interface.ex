defmodule EctoInterface.Paginator.ConfigTest do
  use EctoInterface.DataCase

  describe "Config.new/2" do
    test "creates a new config" do
      config = EctoInterface.Paginator.Config.new(cursor_fields: [:id], limit: 10)
  ...
  describe "Config.new/2 applies sort_fields" do
    test "applies column fields with default direction" do
      config = EctoInterface.Paginator.Config.new(cursor_fields: [:id])

      assert config.cursor_fields == [id: :asc]
    end

    test "applies column fields with alternate direction" do
      config = EctoInterface.Paginator.Config.new(cursor_fields: [:id], sort_direction: :desc)
    ...
    test "applies column with direction tuples" do
      config =
        EctoInterface.Paginator.Config.new(cursor_fields: [id: :desc], sort_direction: :asc)
    ...
    test "applies column with direction tuples mixed with column fields" do
      config =
        EctoInterface.Paginator.Config.new(
          cursor_fields: [{:id, :desc}, :name],
          sort_direction: :asc
        )
    ...
    test "applies {binding, column} tuples with direction" do
      config =
        EctoInterface.Paginator.Config.new(
          cursor_fields: [{{:payments, :id}, :desc}],
          sort_direction: :asc
        )
  ...
  describe "Config.new/2 applies min/max limit" do
    test "applies default limit" do
      config = EctoInterface.Paginator.Config.new()
      assert config.limit == 50
      assert config.total_count_primary_key_field == :id
    end

    test "applies minimum limit" do
      config = EctoInterface.Paginator.Config.new(limit: 0)
      assert config.limit == 1
    end

    test "applies maximum limit" do
      config = EctoInterface.Paginator.Config.new(limit: 1000)
      assert config.limit == 500
    end

    test "respects configured maximum limit" do
      config = EctoInterface.Paginator.Config.new(limit: 1000, maximum_limit: 2000)
      assert config.limit == 1000

      config = EctoInterface.Paginator.Config.new(limit: 3000, maximum_limit: 2000)
  ...
  describe "Config.new/2 decodes cusors" do
    ...
    test "simple after" do
      config =
        EctoInterface.Paginator.Config.new(limit: 10, cursor_fields: [:id], after: simple_after())
  ...
  describe "Config.validate!/1" do
    test "raises ArgumentError when cursor_fields are not set" do
      ...
      assert_raise EctoInterface.Paginator.Config.ArgumentError,
                   "expected `:cursor_fields` to be set",
                   fn ->
    ...
    test "raises ArgumentError when after cursor does not match the cursor_fields" do
      config = EctoInterface.Paginator.Config.new(cursor_fields: [:date], after: complex_after())
    ...
    test "raises ArgumentError when before cursor does not match the cursor_fields" do
      config = EctoInterface.Paginator.Config.new(cursor_fields: [:id], before: complex_before())
  ...
  def simple_after, do: EctoInterface.Paginator.Cursor.encode(%{id: "pay_123"})
  def simple_before, do: EctoInterface.Paginator.Cursor.encode(%{id: "pay_789"})

  def complex_after,
    do: EctoInterface.Paginator.Cursor.encode(%{date: "2036-02-09T20:00:00.000Z", id: "pay_123"})

  def complex_before,
    do: EctoInterface.Paginator.Cursor.encode(%{date: "2036-02-09T20:00:00.000Z", id: "pay_789"})
end
