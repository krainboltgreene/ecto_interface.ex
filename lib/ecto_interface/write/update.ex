defmodule EctoInterface.Write.Update do
  ...
  defmacro __using__([schema, singular])
    ...
    quote(location: :keep) do
      ...
      @doc """
      Applies a set of `attributes` to the given `record`, a `#{unquote(schema)}`, via
      `#{unquote(:"change_#{singular}")}/2` with the `changeset` function, then updates the database with the
      ...
      @spec unquote(:"update_#{singular}")(
              ...
              {:ok, unquote(schema).t()} | {:error, Ecto.Changeset.t(unquote(schema).t())}
      def unquote(:"update_#{singular}")(record, changeset, attributes, preload: preload)
          when is_struct(record, unquote(schema)) and is_function(changeset, 2) and
                 is_map(attributes),
          do:
            record
            |> Application.get_env(:ecto_interface, :default_repo).preload(preload)
      ...
      """
      @spec unquote(:"update_#{singular}")(unquote(schema).t(), function(), map()) ::
              {:ok, unquote(schema).t()} | {:error, Ecto.Changeset.t(unquote(schema).t())}
      def unquote(:"update_#{singular}")(record, changeset, attributes)
          when is_struct(record, unquote(schema)) and is_function(changeset, 2) and
                 is_map(attributes),
          do:
            record
