defmodule EctoInterface.Paginator.Config do
  ...
  def new(opts \\ []) do
    %__MODULE__{
      after: opts[:after],
      after_values: EctoInterface.Paginator.Cursor.decode(opts[:after]),
      before: opts[:before],
      before_values: EctoInterface.Paginator.Cursor.decode(opts[:before]),
      cursor_fields: opts[:cursor_fields],
      fetch_cursor_value_fun:
        opts[:fetch_cursor_value_fun] || (&EctoInterface.Paginator.default_fetch_cursor_value/2),
  ...
  def validate!(%__MODULE__{} = config) do
    unless config.cursor_fields do
      raise(EctoInterface.Paginator.Config.ArgumentError,
        message: "expected `:cursor_fields` to be set"
      )
    end
